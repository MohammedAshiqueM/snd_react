...
export default function Tags() {
    ...
    // Render tags or shimmer
    const renderTags = () => {
        if (isLoading) {
            return (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[...Array(tagsPerPage)].map((_, index) => (
        ...
        if (tags.length === 0) {
            return <p className="text-center">No tags found.</p>;
        }
                ...
                >
                    <div className="flex justify-between items-center mb-6">
                        <div className="text-sm text-gray-500 mb-4">
                            {isLoading ? 'Loading tags...' : `${tags.length} tags ${searchQuery && `matching "${searchQuery}"`}`}
                        </div>
                    </div>
